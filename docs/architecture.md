...
| `migrate(doc)` | Validate a loaded save's `schemaVersion` |
...
3. Loop until `now`. Each segment ends at the earliest of: `now`, the next whole game hour, an op's `completesAt`, `bribeUntil` (while a bribe is active), `recruitPool.refreshAt`, `rival.tolya.nextTickAt`, or a jailed crew member's `jailedUntil`.
...
5. **At a whole hour:** update front utilization and racket condition, then the inspection flag and the raid and arrest rolls. At a day start, also settle wages, drift loyalty, and roll walkouts.
6. **Then events due at the boundary**, in fixed order: ops completing (by time, then id), bribe expiry, jail releases, recruit pool refresh, Tolya's visit.
...
`engine/model/state.ts` defines `PlayerState`: currencies (`vault`, `dirty`, `clean`, `influence`, `reputation`), `act`, `heat` and `inspected`, `rackets`, `fronts`, `crew`, `recruitPool`, `ops`, `districts`, `officials`, bribe fields, `wagesOwed`, `influenceToday`, `rival.tolya`, `tutorial`, the event `log`, and playtest `stats`. `SCHEMA_VERSION` is 1. `migrate()` accepts only the current version today; a shape change bumps the version and adds a step there.
