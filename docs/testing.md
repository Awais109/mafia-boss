...
| File | Guards |
|---|---|
| `tests/helpers.ts` | Shared setup: default `config`, `H` (one game hour), `T0` (a start time off the hour, so boundaries fall mid-segment), `quiet` (defaults with no condition decay and no Tolya hits or demands), `fresh()`, `act()` (apply actions, throw on rejection), `crewNamed()`, `expectClose()` (deep equality with relative float tolerance) |
...
| `tests/sim.test.ts` | **Pacing guard**: the casual bot over seeds 42–46 for 8 days keeps Act I at a mean of ≤ 2.1 days, Act II 3–5 days after Act I, heat 25–35, at most one raid, no missed wages, partial outcomes 40–60%; the goldRush bot, spending every bar, still takes a day or more over Act I |
...
| `tests/migrate.test.ts` | A schema 1 save migrates through every step (inbox, board, ledger; crew progress, front dials, haggle state; stock, upkeep, Station Square; gold), keeps old stats, and keeps playing; a save from a newer build is refused |
